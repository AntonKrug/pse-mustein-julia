...
namespace Antmicro.Renode.Peripherals.Video {
  ...
  public class MusteinGenericGPU : AutoRepaintingVideo, IDoubleWordPeripheral, IKnownSize, IMapped {
    ...
    public MusteinGenericGPU(Machine machine, bool registers64bitAligned = false, bool lookupTableRgbx = true, 
                             uint controlBit = 23, uint frameBufferSize = 0x800000) : base(machine) {
      ...
      MemSet(alignedPointer, (byte)0, this.frameBufferSize); // populate the buffer with zeros
    ...
    protected override void Repaint() {
      Action command;
      if (copyPatterns.TryGetValue(Tuple.Create(colorMode, is64bitAligned, pixelPacking), out command)) {
        lock (sync) {
          command.Invoke();
        }
      } else {
    ...
    protected void ConvertAndSkip(int bytesToCopy, int bytesToSkip) {
      int indexDest = 0;
      int indexSrc = 0;
    ...
    private class MusteinSegment : IMappedSegment {
      ...
      public ulong StartingOffset => checked(0); // We are hardcoding the offset to 0
