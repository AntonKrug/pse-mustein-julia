...
void MUSTEIN_INLINE mustein_write_low_color_rgb(uint64_t base, uint64_t offset,
    ...
    *((uint64_t*) base + offset) = (blue >> 6) << 6 | (green >> 5) << 3 | (red >> 5);
}


void MUSTEIN_INLINE mustein_write_high_color_raw16(uint64_t base, uint64_t offset,
        uint16_t value) {
...
void MUSTEIN_INLINE mustein_write_high_color_rgb(uint64_t base, uint64_t offset,
    ...
    *((uint64_t*) base + offset) =  (red >> 3) << 11 | (green >> 2) << 5 | (blue >> 3);
}


// A 24-bit RGB true color value will be shifted into 32-bit RGBX format
void MUSTEIN_INLINE mustein_write_true_color_raw24(uint64_t base, uint64_t offset,
        uint32_t value) {
