...
uint8_t  L2_METADAT_UNCORR_IRQHandler(void);
...
uint8_t  reserved_99_plic_IRQHandler(void);
uint8_t  reserved_100_plic_IRQHandler(void);
uint8_t  reserved_101_plic_IRQHandler(void);
uint8_t  reserved_102_plic_IRQHandler(void);
uint8_t  reserved_103_plic_IRQHandler(void);
...
typedef enum
{
	...
	USOC_AXIC_US_PLIC		=101 + OFFSET_TO_MSS_GLOBAL_INTS,
	USOC_AXIC_DS_PLIC		=102 + OFFSET_TO_MSS_GLOBAL_INTS,
 ...
 typedef struct
{
    ...
    /*-------------------- Source Priority --------------------*/
    volatile uint32_t SOURCE_PRIORITY[PLIC_NUM_SOURCES];                         //0x04
...
#define TARGET_OFFSET_HART0_M 0
#define TARGET_OFFSET_HART1_M 1
#define TARGET_OFFSET_HART1_S 2
#define TARGET_OFFSET_HART2_M 3
#define TARGET_OFFSET_HART2_S 4
#define TARGET_OFFSET_HART3_M 5
#define TARGET_OFFSET_HART3_S 6
#define TARGET_OFFSET_HART4_M 7
#define TARGET_OFFSET_HART4_S 8
...
static inline void PLIC_init(void)
{
    ...
    for(inc = 0U; inc < PLIC_SET_UP_REGISTERS; ++inc)
    {
    ...
    * the priorities previously set by other harts to be messed up. */
    ...
    {
        PLIC->SOURCE_PRIORITY[inc] = 0U; /* priority must be greater than threshold to be enabled, enable while testing */
    }
...
static inline void PLIC_EnableIRQ(PLIC_IRQn_Type IRQn)
{
	...
	{
		case 0:
		    current = PLIC->HART0_MMODE_ENA[IRQn / 32];
    current |= (uint32_t)1 << (IRQn % 32);
    PLIC->HART0_MMODE_ENA[IRQn / 32] = current;
			break;
		case 1:
		    current = PLIC->HART1_MMODE_ENA[IRQn / 32];
		    current |= (uint32_t)1 << (IRQn % 32);
		    PLIC->HART1_MMODE_ENA[IRQn / 32] = current;
			break;
		case 2:
		    current = PLIC->HART2_MMODE_ENA[IRQn / 32];
		    current |= (uint32_t)1 << (IRQn % 32);
		    PLIC->HART2_MMODE_ENA[IRQn / 32] = current;
			break;
		case 3:
		    current = PLIC->HART3_MMODE_ENA[IRQn / 32];
		    current |= (uint32_t)1 << (IRQn % 32);
		    PLIC->HART3_MMODE_ENA[IRQn / 32] = current;
			break;
		case 4:
		    current = PLIC->HART4_MMODE_ENA[IRQn / 32];
		    current |= (uint32_t)1 << (IRQn % 32);
		    PLIC->HART4_MMODE_ENA[IRQn / 32] = current;
			break;
		default:
			break;
	}
 ...
 * 	EXT_IRQ_DISABLE to return from the external interrupt handler.
 */
...
{
    return (PLIC->PENDING_ARRAY[IRQn/32] & (0x01<<(IRQn%32)));
}
 ...
 *
 */
//PLIC_ClearPendingIRQ() TODO: Place holder, will be added in future release
